  ...
  moduleServer(id, function(input, output, session) {
    ...
    dispersion_results <- eventReactive(input$run_dispersion, {
      ...
      withProgress(message = 'Calculating dispersion...', value = 0, {
        ...
        # Count occurrences.
        # Three cases:
        #   1. regex / tagged-regex search -> grepl per token
        #   2. single-word exact search    -> token == term
        #   3. multi-word phrase           -> match a consecutive token sequence
        phrase_parts <- strsplit(search_pattern, "\\s+")[[1]]
        is_phrase <- !use_regex && length(phrase_parts) > 1
        
        # Count non-overlapping... actually overlapping occurrences of a token
        # sequence `pat` within a token vector `tokens`.
        count_sequence <- function(tokens, pat) {
          np <- length(pat)
          nt <- length(tokens)
          if (np == 0 || nt < np) return(0L)
          hits <- 0L
          # Compare the whole vector shifted, position by position in the phrase.
          idx <- seq_len(nt - np + 1L)
          match_all <- rep(TRUE, length(idx))
          for (k in seq_len(np)) {
            match_all <- match_all & (tokens[idx + (k - 1L)] == pat[k])
          }
          sum(match_all)
        }
        ...
        if (use_regex) {
          ...
          # Phrase search: tokens are already lowercased; phrase_parts from a
          # lowercased search_term, so a direct sequence match is case-insensitive.
          frequencies <- sapply(grouped_tokens, function(tokens) {
            count_sequence(tokens, phrase_parts)
          })
        } else if (is_tagged_search) {
          frequencies <- sapply(grouped_tokens, function(tokens) {
            sum(tolower(tokens) == tolower(search_pattern))
          })
        } else {
          frequencies <- sapply(grouped_tokens, function(tokens) {
    ...
    output$dispersion_barcode <- renderPlot({
      ...
      for (i in seq_along(tokens_lower)) {
        ...
        # Find match positions. For a multi-word phrase, return the start
        # index of each consecutive-sequence match; otherwise single-token.
        if (use_regex) {
          matches <- grep(search_term, tokens, perl = TRUE, ignore.case = TRUE)
        } else {
          phrase_parts <- strsplit(search_term, "\\s+")[[1]]
          if (length(phrase_parts) > 1) {
            np <- length(phrase_parts)
            nt <- length(tokens)
            if (nt >= np) {
              idx <- seq_len(nt - np + 1L)
              match_all <- rep(TRUE, length(idx))
              for (k in seq_len(np)) {
                match_all <- match_all & (tokens[idx + (k - 1L)] == phrase_parts[k])
              }
              matches <- idx[match_all]
            } else {
              matches <- integer(0)
            }
          } else {

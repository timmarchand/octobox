...
tokenSummaryServer <- function(id, token_data, meta_filter) {
  moduleServer(id, function(input, output, session) {
      ...
      withProgress(message = 'Calculating corpus statistics...', value = 0, {
        ...
        if (view_type == "corpus") {
          ...
          result_df <- data.frame(
            ...
            texts = nrow(base_df),
            tokens = total_tokens,
            mean_tokens = round(total_tokens / nrow(base_df), 1),
            types = total_types,
          ...
          for (meta_group in unique(meta_info)) {
            ...
            result_list[[meta_group]] <- data.frame(
              ...
              mean_tokens = round(group_total_tokens / length(group_docs), 1),
          ...
          result_df <- base_df %>%
            ...
            mutate(texts = 1, mean_tokens = tokens) %>%
            select(category, texts, tokens, mean_tokens, types, ttr, sampled_ttr, ttr_samples)
        }
    ...
    output$token_summary <- DT::renderDT({
      ...
      # Drop the internal TTR-sample count from the display (it's the number of
      # 400-word windows used, capped at 200 - almost always 200, so it adds
      # nothing for the reader). Mean Tokens is shown instead.
      data <- data[, setdiff(names(data), "ttr_samples"), drop = FALSE]
      ...
      DT::datatable(
        ...
        colnames = c("Category", "Texts", "Tokens", "Mean Tokens", "Types", "TTR", "Sampled TTR-400"),
        caption = htmltools::tags$caption(
          ...
          paste("Corpus summary statistics showing vocabulary richness measures.",
                "Mean Tokens = average text length. TTR = Type-Token Ratio.",
                "Sampled TTR uses random 400-word windows for length normalization.")
        )
      ) %>%
